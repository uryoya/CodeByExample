...
///
/// If `s` has the proper from , return `Some<(x, y)>`. If it doesn't parse
...
/// Render a rectangle of te Mandelbrot set into a buffer of pixels.
...
fn main() {
    ...
    let bounds = parse_pair(&args[2], 'x')
        .expect("error parsig image dimensions");
